...
public class MissingReferencesFinder : MonoBehaviour
{
    ...
    private static void FindMissingReferences(string context, GameObject[] gameObjects)
    {
        ...
        foreach (var go in gameObjects)
        {
            ...
            foreach (var component in components)
            {
                ...
                if (!component)
                {
                    Debug.LogErrorFormat(go, $"Missing Component {0} in GameObject: {1}", component.GetType().FullName,
                        GetFullPath(go));
                ...
                while (sp.NextVisible(true))
                {
                    if (sp.propertyType == SerializedPropertyType.ObjectReference)
                    {
                                ...
                                objectReferenceStringValue.StartsWith("Missing")))
                        {
                            ...
                            Image c = component as Image;
                            if (c != null)
                            {
                            }
                            ShowError(context, go, component.GetType().Name, propertyName);
                        }
                    }
                }
            }
    ...
    private static void ShowError(string context, GameObject go, string componentName, string propertyName)
    {
        ...
        Debug.LogError(string.Format(ERROR_TEMPLATE, go.name, componentName, propertyName, context), go);
    }
