...
- The console now uses SettingsProvider rather than the now-deprecated PreferenceItem

Bug fixes
- Created a new attribute for console commands called [ConsoleCommand]. This works exactly the same way as [CommandHandler], but avoids a conflict with a new class built into Unity called CommandHandler which was causing errors.  
...
- Adds support for CommandHandlers that have arguments that are unsinged ints, shorts and longs. Also adds support for byte and char arguments.

== Version 2.7.1 ==
New features
- Added overloaded version of DebugConsole.Instance.ExportLog that takes a Stream object. Useful if you want to upload log data to your server but don't want to write it to disk first. Bear in mind that the Stream needs to stay open until callback returns, so can't be opened with using(...).

Bug fixes
- Added the NativeMethods class to link.xml - this is called by reflection so errors can happen if this class gets stripped.
- Fix for upgrade logic incorrectly moving too many iOS files into the console's directory.
